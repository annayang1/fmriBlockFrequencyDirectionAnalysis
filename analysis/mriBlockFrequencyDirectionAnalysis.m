...
%    [] - load and process stim/response files, do not save packets
%    'save' - load and process stim/response files, save the packets
%    'load' - load the packets from the passed hash name
%    'compare' - load and process stim/response files, compare the hash of
%        the resulting packets to the passed hash
packetCacheBehavior='save';
packetCachePassedHash=[];

% Discover user name
[~, userName] = system('whoami');
userName = strtrim(userName);

% If we are not to load the packetCellArray, then we must generate it
if ~strcmp(packetCacheBehavior,'load')
    ...
    packetCellArray_MD5Hash = HashData(packetCellArray);
end
...
if strcmp(packetCacheBehavior,'save')
    ...
    dropboxDataDir = fullfile('/Users', userName, '/Dropbox (Aguirre-Brainard Lab)/MELA_analysis/mriBlockFrequencyDirectionAnalysis/packetCache');
    packetCacheFileName=fullfile(dropboxDataDir, packetCellArray_MD5Hash, '.mat');
